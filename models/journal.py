...
def short_name(code, name):
    ...
    # الاسم يُعاد **كاملاً**. كان يُقتطع إلى كلمتين حتى لا يتمدّد
    # العمود أفقياً — لكن ذلك يخفي بقية الاسم فلا يُعرف صاحب الحركة،
    # وهو أهم ما في العمود. الحل في العرض لا في البيانات: الخلية
    # تلفّ الاسم على أسطر داخل عرضها الثابت (انظر `_fit_rows` في
    # ui/general_ledger_screen.py).
    return n or "—"
...
def statement(conn, account_id, date_from=None, date_to=None):
    ...
    for r in conn.execute(q, params):
        ...
        if merged and merged[-1]["_eid"] == key:
            ...
            continue
        # ══ البيان = ما كتبه المستخدم، لا شيء غيره ══
        # كان يُملأ بوصف السطر الآلي حين يخلو من نصّ المستخدم
        # («مديونية ذهب وأجور» · «خروج الذهب من المخزون»)، فيمتلئ
        # العمود بجملٍ يعرفها النظام ولا تفيد قارئ الكشف، ويضيع
        # فيها السطر الذي كتبه المحاسب بيده. وكانت الفواتير تُستثنى
        # فيُمحى بيانها أصلاً — فمن كتب بياناً في شاشة المبيعات لم
        # يجده في الكشف إطلاقاً.
        #
        # القاعدة الآن واحدة لكل مستند: البيان نصُّ المستخدم
        # (`user_note`) إن كتبه، وإلا فراغ. واسم الجهة المقابلة له
        # عموده، ونوع العملية له عموده — فلا يحتاج البيان أن ينوب
        # عنهما.
