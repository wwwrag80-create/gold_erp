# -*- coding: utf-8 -*-
"""أعمار المخزون — كم من رأس المال راكدٌ، ومنذ متى.
...
**والأجرة الراكدة** هي الرقم الذي يفهمه صاحب المصنع: ليست ربحاً
ضائعاً بل **أجرةً صُرفت ولم تُحصَّل** — الوزن × أجرة الجرام المسجّلة
على الأمر. تُقرأ لقياس الركود، ولا تدخل قائمة دخلٍ ولا قيداً.
...
def report(conn, as_of=None, model=None):
    """أعمار المخزون: الفئات، والموديلات، والقطع، وأقدمها."""
    as_of = as_of or _today()
...
def verdict(r, fmt=None, money=None):
    ...
    out.append(
        f"فوق التسعين يوماً: {fmt(r['old_weight'])} ({r['old_pct']:,.1f}%) "
        f"وأجرةٌ راكدة {money(r['buckets'][-1]['idle_wage'])} ريال.")
    if r["old_pct"] >= 50:
        out.append(
            "أكثر من نصف المخزون راكدٌ فوق التسعين — هذا ذهبٌ لا يدور "
            "وأجورٌ صُرفت ولم تُحصَّل. راجع التسعير أو أعد الصهر قبل "
            "شراء ذهبٍ جديد.")
    elif r["old_pct"] >= 30:
