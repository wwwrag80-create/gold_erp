...
def main():
    ...
    # المصنع النشط = الأحدث استعمالاً. إن كان هو نفسه صاحب البيانات
    # فالنظام يفتح القاعدة الصحيحة أصلاً ولا شيء يُنقل — وأي نقل هنا
    # يستبدل مصنعاً آخر بلا داعٍ. (كان الاستنتاج السابق «الأحدث بعد
    # المصدر» فيقترح هدفاً خاطئاً في هذه الحالة تحديداً.)
    newest = rows[0]
    if newest["id"] == src["id"]:
        print("\n" + "═" * 66)
        print("  ✔ النظام يفتح قاعدتك الصحيحة أصلاً — لا شيء يُنقل.")
        print(f"     المصنع النشط: {src['id']}")
        print(f"     {fmt(src['counts'])}")
        print("\n  إن كانت البيانات لا تظهر في الشاشات رغم ذلك، فالسبب")
        print("  ليس مكان الملف. شغّل:")
        print("      python tools/fix_dates.py")
        print("  التواريخ المكتوبة بأرقام عربية تسقط من كل فلتر تاريخ،")
        print("  فتكون القيود موجودة وغير مرئية.")
        print("═" * 66)
        return 0
    target = newest
