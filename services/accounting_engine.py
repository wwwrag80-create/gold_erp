...
def post_entry(conn, entry_date, description, lines,
    ...
    # الرصيد السالب في مصنعٍ يعمل حالةٌ واقعية (بضاعة تخرج قبل تسجيل
    # توريدها)، فكان التنبيه يتكرّر على عملياتٍ سليمة. وما يكشف
    # السالب الحقيقي باقٍ حيث ينفع: «أرصدة المخازن» و«صحة النظام»
    # و«ميزان المراجعة» تقرأ الأرصدة كما هي.
