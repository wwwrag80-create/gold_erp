    ...
    def load(self):
        try:
            with db() as conn:
                ...
                try:
                    ...
                    inv_g = ((pin.get("jewels") or 0)
                             + (pin.get("stones") or 0))
            self.t_gold.set_value(f"{pin['gold']:,.2f} جم",
            ...
            rows = [
                ...
                ("إجمالي الذهب الوارد (صب/كسر/تحويل داخلي...)", f"{r['gold_in']:,.2f}"),
                ("إجمالي الفصوص والأحجار الواردة (عبر قيود اليومية)",
                 f"{r['jewels_in']:,.2f}"),
                ("إجمالي المدخلات", f"{r['total_in']:,.2f}"),
            ...
            fill(self.table, ["البند", "جرامات (عيار 18)"], rows)
            check = round(r["opening"] + r["total_in"] - r["production"]
                         + r["other"] - r["closing"], 3)
            ok = abs(check) < 0.005
            self.summary.setText(
                f"مدخلات الفترة: {r['total_in']:,.2f} جم (منها {r['jewels_in']:,.2f} "
                f"فصوص وأحجار) | مخرجات الإنتاج: {r['production']:,.2f} جم | "
